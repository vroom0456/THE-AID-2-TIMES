...
function Loader() {
  return (
    <div style={{
      ...
      <div style={{
        width: 160, height: 1, background: "#2a2a2a",
        position: "relative", overflow: "hidden",
      }}>
        <div style={{
          position: "absolute", top: 0, left: 0, height: "100%",
          width: "60%", background: "#f5f5f5",
...
function Cursor() {
  ...
  useEffect(() => {
    ...
    const onHoverIn  = () => { dotRef.current?.classList.add("hov");  ringRef.current?.classList.add("hov"); };
    const onHoverOut = () => { dotRef.current?.classList.remove("hov"); ringRef.current?.classList.remove("hov"); };
  ...
  return (
    <>
      <div className="cur" id="cur" ref={dotRef} />
      <div className="cur-ring" id="curRing" ref={ringRef} />
    </>
  );
}
...
export default function App() {
  ...
  useEffect(() => {
    const handler = (e) => {
      if ((e.ctrlKey || e.metaKey) && e.key === "k") { e.preventDefault(); useUIStore.getState().setKBOpen(true); }
  ...
  useEffect(() => {
    const push = () => window.history.pushState({ ait2: true }, "", window.location.href);
    push();
    const onBack = () => {
      const store = useUIStore.getState();
      if (store.kbOpen)        { store.setKBOpen(false);           push(); return; }
      if (store.modals?.dash)  { store.setModalOpen("dash", false); push(); return; }
      if (store.modals?.calc)  { store.setModalOpen("calc", false); push(); return; }
      push();
    };
